...
const Sidebar = ({ expand, setExpand }) => {
  return (
    <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all duration-500 z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
      <div>
        {/* Logo + Menu Toggle */}
        <div onClick={() => setExpand(!expand)} className={`flex ${expand ? 'flex-row gap-10' : 'flex-col items-center gap-8'}`}>
      ...
      <div className={`flex items-center cursor-pointer group relative ${expand ? "gap-1 text-white/80 text-sm p-2.5 border border-primary rounded-lg hover:bg-white/10" : "h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
        ...
        {/* Optional text/icon beside icon when expanded */}
        {expand && (
          <>
            <span>Get App</span>
